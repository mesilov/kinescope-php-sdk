...
### OpenSpec Workflow
Use OpenSpec for behavior, API, CLI, architecture, and public SDK contract changes.
- Before implementation, create or update a change under `openspec/changes/<change-id>/`.
- Keep requirements in delta specs under `openspec/changes/<change-id>/specs/<capability>/spec.md`.
- Use `proposal.md` for intent and scope, `tasks.md` for implementation checklist, and `design.md` when the change has non-trivial architecture or trade-offs.
- Validate OpenSpec artifacts with `make openspec-validate` before implementing and before final review.
- Small documentation-only, formatting-only, generated-code-only, and local tooling maintenance changes may skip an OpenSpec change when they do not alter SDK behavior or contracts.

OpenSpec commands are wrapped by Makefile and run through Docker:
- `make openspec args="list"` — run arbitrary OpenSpec CLI arguments.
- `make openspec-list` — list active changes.
- `make openspec-list-specs` — list current specs.
- `make openspec-validate` — strict non-interactive validation of all changes and specs.

Repository-local OpenSpec Codex skills are generated by `openspec init --tools codex` under `.codex/skills/`:
- `$openspec-explore` — explore ideas and code context before creating a change.
- `$openspec-propose` — create proposal, design, specs, and tasks.
- `$openspec-apply-change` — implement an approved change.
- `$openspec-archive-change` — archive completed changes.

These generated skills mention direct `openspec ...` commands. In this repository, run OpenSpec through the Makefile wrappers above unless the CLI is explicitly installed in the current shell.
